...
const WhyFlowzenSection = () => {
  ...
  useEffect(() => {
    if (isMobile || isTablet) {
      ...
      setMouseEnteredTwo(true);
      setMouseEnteredThree(true);
      setMouseEnteredFour(true);
    } else {
      setMouseEntered(false)
    }
  }, []);
  return (
    <motion.section initial={{ opacity: 0, y: 50 }} animate={inView ? { opacity: 1, y: 0 } : { opacity: 0, y: 50 }} transition={{ duration: 1.3 }} ref={ref} className='w-full lg:h-[80vh] flex flex-col p-5 gap-7'>
      ...
      <div className='w-full h-full overflow-hidden max-sm:flex max-sm:flex-col max-sm:gap-5'>
